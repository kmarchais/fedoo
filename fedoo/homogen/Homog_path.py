...
if USE_SIMCOON:
    ...
    def SolverUnitCell(
        ...
        if meshperio:
            DefinePeriodicBoundaryCondition(
                mesh,
                [
                    StrainNodes[0],
                    StrainNodes[0],
        ...
        for blocknumber, block in enumerate(blocks):
            for stepTotNumber, step in enumerate(block):
                ...
                for i, dt in enumerate(step.times):
                    ...
                    initValue[step.cBC_meca.astype(bool)] = MeanStress[
                        step.cBC_meca.astype(bool)
                    ]
                    ...
                    pb.nlsolve(
                        dt=dt * step.Dn_init,
                        dt_min=dt * step.Dn_init * step.Dn_mini,
                        tmax=dt,
                        update_dt=True,
                        ToleranceNR=0.05,
                        intervalOutput=2.0 * dt,
                    )
                    ...
                    MeanStress = np.array(
                        [
                            1
                            / Volume
                            * Assembly.get_all()["Assembling"].integrate_field(
                                TensorStress[i],
                            )
                            for i in range(6)
                        ],
                    ...
                    Wm_mean = (1 / Volume) * Assembly.get_all()[
                        "Assembling"
                    ].integrate_field(material.Wm)
    ...
    def get_resultsUnitCell(
        ...
        BlocksCyclesSteps_df = pd.DataFrame(
            np.array(Res[0]),
            columns=BlocksCyclesSteps_columns,
        )
        ...
        if Tangent_bool:
            ...
            TangentMatrix_df = pd.DataFrame(
                np.array(Res[4]),
                columns=TangentMatrix_columns,
            )
            Results_df = pd.concat(
                [
                    BlocksCyclesSteps_df,
                    MeanStrain_df,
                    MeanStress_df,
                    Wm_df,
                    TangentMatrix_df,
            ...
            Results_df = pd.concat(
                [BlocksCyclesSteps_df, MeanStrain_df, MeanStress_df, Wm_df],
                axis=1,
            )
