...
def import_msh(
    filename: str,
    name: str = "",
    mesh_type: list[str] = ["curve", "surface", "volume"],
) -> Mesh:
        ...
        if l == "$physicalnames":
            ...
            if l == "$nodedata":
                ...
                if len(msh[0].split()) == 2:
                    ElmData += [
                        np.array(
                            [msh[el].split()[1] for el in range(Nb_el)],
                            dtype=float,
                        ),
                    ]
                elif len(msh[0].split()) > 3:
    ...
    if version == "2.2":
        ...
        for celltype in list(np.unique(celltype_all)):
            ...
            if type_elm is None:
                print(
                    f"Warning : Elements type {celltype} is not implemeted!",
                )  # element ignored
            else:
                if multi_mesh:
...
def import_vtk(filename: str, name: str = "") -> Mesh:
    ...
    if fileversion != "#vtkdatafileversion2.0":
