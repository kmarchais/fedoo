...
def extract_surface(mesh):
    ...
    while i < len(sorted_faces):
        if (sorted_faces[i] != sorted_faces[i - 1]).any():
            ...
            while (
                i < len(sorted_faces) and (sorted_faces[i] == sorted_faces[i - 1]).all()
            ):
                i += 1
        i += 1
    ...
    if dim == 1:
        local_frame = element_face.GetLocalFrame(
            elm_nodes_crd,
            element_face.get_gp_elm_coordinates(1),
        )
        ...
        vec_xi = (
            1
            / length
            * ((mesh.nodes[nodes_inside] - elm_nodes_crd[:, 0, :]) * tangent_axis).sum(
                axis=1,
            )
        )
    else:  # dim == 2
...
def change_elm_type(mesh, elm_type, name=""):
    ...
    new_nodes_ind = np.arange(
        mesh.n_nodes,
        mesh.n_nodes + mesh.n_elements * new_nodes_crd.shape[1],
    )
