...
def write_vtk(dataset, filename="test.vtk", gp_data_to_node=True):
    ...
    if cell_type == None:
        raise NotImplementedError(f"{type_elm} is not available in vtk")
    ...
    # POINT
    ret += [f"DATASET {datatype}", f"POINTS {mesh.n_nodes} float"]
    ...
    # CELLS
    ret += [f"CELLS {len(elm)} {(np.array(np.shape(elm))+np.array([0,1])).prod()}"]
    ret += [" ".join([nb_nd_elm] + [str(nd) for nd in line]) for line in elm]
    ret += [f"CELL_TYPES {len(elm)}"]
    ...
    if len(node_data) > 0:
        ...
        for data_name, data in node_data.items():
            ...
            if len(np.shape(data)) == 1:  # Scalar data
                ret += [f"SCALARS {data_name} float 1"]
    ...
    if len(dataset.element_data) > 0:
        ret += [f"CELL_DATA {mesh.n_elements}"]
...
def write_msh(dataset, filename="test.msh", gp_data_to_node=True):
    ...
    if type_el == None:
        raise NotImplementedError(f"{type_elm} is not available in msh")
