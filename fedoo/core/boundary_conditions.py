...
class BCBase:
    ...
    def str_condensed(self):
        ...
        if self.name == "":
            return f"{self.bc_type}"
        else:
            return f"{self.bc_type} (name = '{self.name}')"
...
class ListBC(BCBase):
    ...
    def __repr__(self):
        ...
        if self.name != "":
            list_str.append(f"name = '{self.name}'")
    ...
    def str_condensed(self):
        """Return a condensed one line str describing the object"""
        if self.name == "":
            return f"ListBC -> list of {len(self)} bc"
        else:
            f"ListBC (name = {self.name}) -> list of {len(self)} bc"
...
class BoundaryCondition(BCBase):
    ...
    def str_condensed(self):
        ...
        if self.name == "":
            ...
            res = [f"{self.bc_type} (name = '{self.name}') -> "]

        res.append(f"'{self.variable_name}' for")

        if isinstance(self.node_set_name, str):
            res.append(f"node_set '{self.node_set_name}'")
        else:  # iterable
            res.append(f"{len(self.node_set_name)} nodes")

        if np.isscalar(self.value):
            res.append(f"set to {self.value}")
        else:
    ...
    def __repr__(self):
        res = [f"{self.bc_type} boundary condition:"]
        if self.name != "":
            res.append(f"name = '{self.name}'")
        res.append(f"var = '{self.variable_name}'")

        if isinstance(self.node_set_name, str):
            res.append(f"node_set = '{self.node_set_name}'")
        else:  # iterable
            res.append(f"n_nodes = {len(self.node_set_name)}")
