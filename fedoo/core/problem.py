...
from fedoo.core.output import _get_results, _ProblemOutput
...
class Problem(ProblemBase):
    ...
    def solve(self, **kargs):
        ...
        if len(self.__A.shape) == 2:  # A is a matrix
            if len(self._dof_slave) == 0:
                print(
                    'Warning: no dirichlet boundary conditions applied. "Problem.apply_boundary_conditions()" is probably missing',
                )
            ...
            assert self.__D != 0, "internal error, contact developper"
    ...
    def apply_boundary_conditions(self, t_fact=1, t_fact_old=None):
        ...
        for e in self.bc.generate(self, t_fact, t_fact_old):
            ...
            if e.bc_type == "MPC":
                ...
                row.append(
                    (
                        np.array(e._dof_index[0]).reshape(-1, 1) * np.ones(n_fact)
                    ).ravel(),
                )
    ...
    def get_ext_forces(self, name="all", include_mpc=True):
        ...
        if self._MFext == 0 or not (include_mpc):
            ...
            if self.get_D() == 0:
                ...
                return self._get_vect_component(
                    M @ self.get_A() @ self.get_X() - M @ self.get_D(),
                    name,
                )
