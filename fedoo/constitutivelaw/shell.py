...
class ShellBase(ConstitutiveLaw):
    ...
    def update(self, assembly, pb):
        ...
        if U == 0:
            ...
            GeneralizedStrain = [
                0 if op == 0 else assembly.get_gp_results(op, U)
...
class ShellHomogeneous(ShellBase):
    ...
    def GetShellRigidityMatrix(self):
        ...
        Hplane = np.array(
            [[Hplane[i][j] for j in [0, 1, 3]] for i in [0, 1, 3]],
            dtype="object",
        )
        Hshear = self.__material.get_elastic_matrix()
        Hshear = np.array(
            [[Hshear[i][j] for j in [4, 5]] for i in [4, 5]],
            dtype="object",
        )
    ...
    def get_stress(self, assembly, **kargs):
        ...
        Stress = [
            sum([0 if Strain[j] == 0 else Strain[j] * Hplane[i][j] for j in range(4)])
            for i in range(4)
        ]  # SXX, SYY, SXY (SZZ should be = 0)
        Hshear = self.__material.get_elastic_matrix()
        Stress += [
            sum([0 if Strain[j] == 0 else Strain[j] * Hshear[i][j] for j in [4, 5]])
            for i in [4, 5]
        ]  # SXX, SYY, SXY (SZZ should be = 0)
...
class ShellLaminate(ShellBase):
    ...
    def GetStressDistribution(self, assembly, pg, resolution=100):
        ...
        Hplane = [
            [
                [
                    0 if Hplane[layer][i][j] == 0 else Hplane[layer][i][j]
        ...
        Hshear = [
            [
                [
                    0 if Hshear[layer][i][j] == 0 else Hshear[layer][i][j]
                    for layer in layer_z
                ]
                for j in [4, 5]
            ]
        ...
        Stress = [
            sum(
                [
                    0 if Strain[j] == 0 else Strain[j] * np.array(Hplane[i][j])
                    for j in range(4)
                ],
            )
            for i in range(4)
        ...
        Stress += [
            sum(
                [
                    0 if Strain[4 + j] == 0 else Strain[4 + j] * np.array(Hshear[i][j])
                    for j in range(2)
                ],
            )
            for i in range(2)
