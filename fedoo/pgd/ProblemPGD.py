...
class ProblemPGD(ProblemBase):
    ...
    def SecTerm_RS(
        ...
        return sp.reshape(
            sp.dot(prod_aux_3, self.__C.data[d1].T) @ self.__MatCB[d1],
            (-1, 1),
        )
    ...
    def update_pgd(self, termToChange, ddcalc="all"):  # extended PGD
        ...
        termToKeep = [t for t in range(self.__X.nbTerm()) if t not in termToChange]
        ...
        for e in self.bc.generate(self, t_fact, t_fact_old):
            ...
            if e.bc_type == "Neumann":
                ...
                if len(SetOfNodesForBC[1]) == 1 and isinstance(
                    Value,
                    (int, float, np.floating, np.ndarray),
                    ...
                    if dimBC is None:  # initialization of Xbc
                        if Value != 0:  # modification of the second term Xbc
                            dimBC = dd
                            Xbc = SeparatedArray(
                                [
                                    np.ones((shapeX[d], 1))
                ...
                if (
                    ...
                    IndexMaster = np.array(
                        [setof[1][0] for setof in SetOfNodesForBC_Master],
                        dtype=int,
                    )
